...
export default class RTCConnection extends EventEmitter{
	...
	static EVENT_CLOSED                 = 'peer:closed';
	static EVENT_STATE_CHANGED                = 'peer:state-changed';
	...
	constructor({tracks, clientId, polite}: RTCConnectionParams) {
		super();
		console.log(this.getMsg(`connect to ${clientId} with polite ${polite}`))
		this.polite = polite;
		...
		this.peerConnection.ontrack = (e: RTCTrackEvent) => {
			...
			try {
				...
				console.log(this.getMsg(`error set mediaCredentials`), e);
			}
		...
		{
			this.emit(RTCConnection.EVENT_STATE_CHANGED);
			console.log(this.getMsg(`state is ${this.peerConnection.iceConnectionState}`))
		...
		this.pc.onnegotiationneeded = async () => {
			console.log(this.getMsg(`negotiation`))
			try {
		...
		// @ts-ignore
		const off = SignalingChannel.onmessage(this.clientId, async ({ description, candidate }) => {
		...
		this.on(RTCConnection.EVENT_CLOSED, off);
	}
	...
	getMsg(s: string) {
		...
		return `[peer:${identifier}] ${s}`;
	}
	close() {

		console.log(this.getMsg('closing'))

		this.emit(RTCConnection.EVENT_CLOSED);
	...
	private removeSender(sender: RTCRtpSender) {
		console.log(this.getMsg(`remove sender`))
		// @ts-ignore
	...
	private addLocalTrack(track: MediaStreamTrack) {
		console.log(this.getMsg(`add local track`))
	...
	private addRemoteTrack(newTrack: MediaStreamTrack) {
		console.log(this.getMsg(`getting remote track`))
		this.tracks.push(newTrack);

		newTrack.onended = () => {
			console.log(this.getMsg(`remote track was finished`))
	...
	removeRemoteTrack(trackId: string) {
		console.log(this.getMsg(`remove remote track`))
		/*
		try {
